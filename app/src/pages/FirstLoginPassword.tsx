  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#F5F8FC] p-5">
      <section className="w-full max-w-lg rounded-xl border border-[#102B4C]/10 bg-white p-7 shadow-[0_24px_70px_rgba(16,43,76,0.08)] sm:p-10">
        ...
        <p className="mt-2 text-[13px] leading-6 text-[#102B4C]/55">Your temporary password can only be used for this first sign-in. Choose at least twelve characters.</p>
        ...
        <form onSubmit={submit} className="mt-7 space-y-5">
          {([
            ...
            <label key={key} className="block text-[11px] font-semibold text-[#102B4C]/65">
              {label}
              <input type="password" required minLength={key === 'currentPassword' ? undefined : 12} value={form[key]} onChange={(event) => setForm({ ...form, [key]: event.target.value })} className="mt-2 w-full rounded-lg border border-[#102B4C]/10 bg-[#F7F9FC] px-4 py-3 text-sm outline-none focus:border-[#060273]/40 focus:ring-2 focus:ring-[#060273]/10" />
            </label>
          ))}
